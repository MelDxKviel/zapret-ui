...
async fn run_elevated_task(
    ...
    match task {
        "service-install" => {
            ...
            // Single hardened entry point: stage the binaries into the ACL-locked
            // machine-wide directory, re-resolve the strategy against it, then
            // install + start the service from THERE — never from the
            // user-writable %APPDATA% dir (privilege-escalation fix).
