...
pub(crate) fn service_image_belongs_to_dirs(image: &OsStr, owned_dirs: &[PathBuf]) -> bool {
    ...
    // Accept both winws2.exe (current zapret2 core) and the legacy winws.exe a
    // pre-migration zapret-ui build may have registered, so an upgrade replaces
    // rather than rejects its own older service.
    if !(name.eq_ignore_ascii_case("winws2.exe") || name.eq_ignore_ascii_case("winws.exe")) {
        return false;
    }
...
/// loads each of them from its working directory at startup. Returns the
/// locked-down machine-wide directory the service should run from.
...
/// ACLs, **re-resolves the strategy against that dir** (so the `winws2.exe` path
/// *and* its `lua/`, `files/`, hostlist file arguments all point at admin-only
...
pub async fn install_service_protected(
    ...
    //   1. A service still running out of the protected dir locks its winws2.exe,
    ...
    // Build the strategy argv rooted at the verified protected copy so the
    // LocalSystem command line (the winws2.exe path *and* every `lua/`/`files/`
    // reference) points only at admin-only locations — never the user-writable
    // %APPDATA% dir. zapret2 strategies are Rust constants, so there is no .bat
    // to re-read: the catalog rebuilds the argv against `protected` on demand.
    let strategy = crate::zapret::strategies::BuiltinCatalog::new(protected.clone())
        .by_id(strategy_id)
        .ok_or_else(|| anyhow::anyhow!("Strategy '{}' not found in catalog", strategy_id))?;
...
impl ServiceCtl for WindowsServiceCtl {
    async fn install(&self, strategy: &Strategy) -> anyhow::Result<()> {
        ...
        // Warn (don't fail) if the strategy's input files aren't all present in
        // the protected install dir — a missing Lua/hostlist/blob would make SCM
        // start the service only to have winws2 die seconds later.
        let missing = crate::zapret::strategies::check_required_files(&self.install_dir, strategy);
        if !missing.is_empty() {
            tracing::warn!(
        ...
        loop {
            match service.query_status() {
                Ok(s) if s.current_state == ServiceState::Stopped => {
                    ...
                    return Err(anyhow::anyhow!(
                        "The zapret service started but winws2.exe exited immediately \
...
mod tests {
    ...
    fn service_image_ownership_respects_path_boundaries() {
        ...
        // winws2.exe (current core) under the owned dir → ours.
        assert!(service_image_belongs_to_dirs(
            OsStr::new(r#""C:\ProgramData\zapret-ui\zapret\winws2.exe" --wf-tcp=80"#),
            &owned,
        ));
        // Legacy winws.exe under the owned dir is still recognised as ours so an
        // upgrade from a pre-migration build replaces rather than rejects it.
        assert!(service_image_belongs_to_dirs(
            OsStr::new(r#""C:\ProgramData\zapret-ui\zapret\winws.exe" --wf-tcp=80"#),
            &owned,
        ));
        // A sibling dir (zapret2) is NOT under the owned `zapret` dir → not ours.
        assert!(!service_image_belongs_to_dirs(
            OsStr::new(r#""C:\ProgramData\zapret-ui\zapret2\winws2.exe" --wf-tcp=80"#),
            &owned,
        ));
    }
}
